...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    d=[0]
    for i, j in enumerate(problem.corners):
    	if not state[1][i]:
    		d.append(manhattandistance(state[0], j))
    return max(d) 
...
def foodHeuristic(state, problem):
    ...
    mindistance=0
    for fc in food:
    	#d=util.manhattanDistance(fc,position) - 9556 nodes
    	
    	d=mazeDistance(fc,position,problem.startingGameState)
    	if d > mindistance:
    		mindistance=d
    return mindistance
