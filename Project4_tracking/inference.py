    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
        	...
        	for newPos, prob in newPosDist.items():
        		updatedBeliefs[newPos] += prob * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while count < self.numParticles:
            for position in self.legalPositions:
            	self.listParticles.append(position)
                count += 1
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            if noisyDistance == None:
    			nbeliefs[self.getJailPosition()] = 1.0
            elif emissionModel[trueDistance] > 0:
                 nbeliefs[p] = (emissionModel[trueDistance] * beliefs[p])
        if not any(nbeliefs.values()):
            self.initializeUniformly(gameState)
        else:
            newParticles = []
            for i in range(0,self.numParticles):
                newParticles.append(util.sample(nbeliefs))
        	self.listParticles = newParticles
...
class JointParticleFilter:
    ...
    def observeState(self, gameState):
        ...
        for distance, particle in enumerate(self.particles):
            belief = 1
            for i in range(self.numGhosts):
                if (noisyDistances[i] == None):
                    ...
                    belief *= emissionModels[i][dist]
            samples[particle] += belief
        if not any(samples.values()):
            ...
            newparticles = []
            for i in range(0,self.numParticles):
                newparticles.append(util.sample(samples))
